...
// FilterOptions filter选项
type FilterOptions struct {
	// DisableRecovery 禁用Recovery, 默认不禁用
	DisableRecovery bool
}

// FilterOption 设置filter选项
type FilterOption func(*FilterOptions)

// LogRecoveryFilter  log recovery filter
func LogRecoveryFilter(opts ...FilterOption) filter.Filter {
	...
	return func(ctx context.Context, req, rsp interface{}, handle filter.HandleFunc) (err error) {
		defer func() {
			if rerr := recover(); rerr != nil {
				...
				// 如果业务方引入了recovery filter, 则在这里recovery不到
