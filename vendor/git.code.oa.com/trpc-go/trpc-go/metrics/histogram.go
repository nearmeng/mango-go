...
// RegisterHistogram 注册名为 name 的 Histogram 的配置到所有实现了 HistogramSink 的 Sink 中。
func RegisterHistogram(name string, o HistogramOption) {
	muxMetricsSinks.Lock()
	defer muxMetricsSinks.Unlock()
...
// HistogramSink 允许在 Sink 中载入名为 name 的 histogram 的 bucket 配置。
// 这相当于是基于 metric 名的对 PolicyHistogram 的扩展。
// 未实现 HistogramSink 的 Sink 将不得不自行决定 bucket 的配置。
type HistogramSink interface {
	Register(name string, o HistogramOption)
}

// HistogramOption 定义了注册 Histogram 时的参数。
