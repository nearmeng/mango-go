...
// CoverGroup 覆盖更新
func (fc *FullCache) CoverGroup(opts types.GetOptions, items []*v3.Item) {
	var tryApp sync.Map
	var tryEnv sync.Map
	...
	for i := 0; i < itemsLen; i++ {
		...
		// 表类型,全量数据才缓存
		if kvItem.StructType == int32(v3.Item_TABLE) {
			if opts.Start != 0 {
				continue
			}
		...
		gw.Version = kvItem.GetVerUuid()
		gw.VersionID = uint64(kvItem.GetVerId())
		gw.VersionName = kvItem.GetVerName()
		gw.StructType = kvItem.GetStructType()

		// table 类型
		if gw.StructType == int32(v3.Item_TABLE) {
			if kvi := CopyKeyValueItem(kvItem); kvi != nil {
				gw.TBL = kvi.GetTables()
			}
			if kvItem.GetEventType() == int32(v3.Item_DELETE) {
		...
		// file类型
		if gw.StructType == int32(v3.Item_FILE) {
			if kvi := CopyKeyValueItem(kvItem); kvi != nil {
				gw.FDL = kvi.GetFiles()
				fc.TypeFile(opts, gw.FDL)
			}
			if kvItem.GetEventType() == int32(v3.Item_DELETE) {
				envTrue.Delete(opts.Group)
			}
		...
		// kv 类型
		if gw.StructType == int32(v3.Item_KV) {
			kvs := kvItem.GetKvs()
			if kvs == nil {
				continue
			}
			for _, kv := range kvs.GetKvs() {
				gw.Value[kv.GetKey()] = kv.GetValue()
			}
		}
	}
	if iv, ok := envTrue.LoadOrStore(opts.Group, &gw); ok {
		group := iv.(*GroupWrapper)
		group.Mutex.Lock()
...
// DeleteGroup 删除group 或者group中的key
// exist indicates whether value was found in the map.
func (fc *FullCache) DeleteGroup(opts types.GetOptions, key string) (exist bool) {
	iv, ok := fc.cache.Load(opts.AppID)
	if !ok {
		return false
	}
	appTrue := iv.(*sync.Map)
	iv, ok = appTrue.Load(opts.EnvName)
	if !ok {
		return false
	}
	envTrue := iv.(*sync.Map)
	iv, ok = envTrue.Load(opts.Group)
	if !ok {
		return false
	}
	...
	group.Mutex.Lock()
	defer group.Mutex.Unlock()
	if _, ok := group.Value[key]; ok {
		delete(group.Value, key)
		return true
	}
	return false
}
...
// UpdateGroup 更新group
func (fc *FullCache) UpdateGroup(opts types.GetOptions, ut UpdateType, items []*v3.Item) {
	var tryApp sync.Map
	var tryEnv sync.Map
	iv, _ := fc.cache.LoadOrStore(opts.AppID, &tryApp)
	appTrue := iv.(*sync.Map)
	...
	for i := 0; i < itemsLen; i++ {
		...
		/*if group.VersionID >= uint64(kvItem.GetVerId()) {
			return
		}*/
		// 表类型,全量数据才缓存
		if kvItem.StructType == int32(v3.Item_TABLE) {
		...
		if i == itemsLen-1 && (ut == UpdateTypeGetGroup || ut == UpdateTypePollGroup) {
			group.Version = kvItem.GetVerUuid()
			group.VersionID = uint64(kvItem.GetVerId())
			group.VersionName = kvItem.GetVerName()
			group.StructType = kvItem.GetStructType()
		}

		// table 类型
		if group.StructType == int32(v3.Item_TABLE) {
			if kvi := CopyKeyValueItem(kvItem); kvi != nil {
				group.TBL = kvi.GetTables()
			}
			if kvItem.GetEventType() == int32(v3.Item_DELETE) {
				envTrue.Delete(opts.Group)
			}
		...
		// file 类型
		if group.StructType == int32(v3.Item_FILE) {
			if kvi := CopyKeyValueItem(kvItem); kvi != nil {
				group.FDL = kvi.GetFiles()
				fc.TypeFile(opts, group.FDL)
			}
			if kvItem.GetEventType() == int32(v3.Item_DELETE) {
				envTrue.Delete(opts.Group)
			}
		...
		// kv 类型
		if group.StructType == int32(v3.Item_KV) {
			kvs := kvItem.GetKvs()
			if kvs == nil {
				continue
			}
			for _, kv := range kvs.GetKvs() {
				if et != int32(v3.Item_DELETE) {
					group.Value[kv.GetKey()] = kv.GetValue()
					continue
				}
				delete(group.Value, kv.GetKey())
			}
		}
	}
}
...
func (fc *FullCache) GetGroup(opts types.GetOptions) (GroupWrapper, error) {
	...
	// 文件类型处理
	if gw.StructType == int32(v3.Item_FILE) {
		gw.FDML = fc.LoadFileMeta(opts, gw.FDL)
	}
	return gw, nil
}
